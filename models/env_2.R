...
#
# File Description:
# Setup of simulation data environment 2

exposure_min <- 5000+2000*((i-1)/n)
exposure_max <- 60000+24000*((i-1)/n)
seed = i
...
min = exposure_min       # Minimum and maximum for effective exposure rates
max = exposure_max
lambda <- c(rep(0.1, I)) # Expected claim frequencies per year

# Set different values for effective exposure rates for each year (E) and
# notification delay (notidel) and settlement delay (setldel), based on short
# or long tailed claims
if (short){
...
# Specify the Weibull parameters (in this case independent of claim_size and
# occurrence_period and different for long and short tails)
...
# Set to low level, at 0.03, of inflation in payment time
