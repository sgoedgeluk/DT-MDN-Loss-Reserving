...
#
# File Description:
# Setup of simulation data environment 1

ref_claim <- 170000+60000*((i-1)/n)
quarterly_exposure = ref_claim
seed = i
...
E <- c(rep(quarterly_exposure, I))   # Effective exposure rates for each quarter
...
# Specify the Weibull parameters (in this case independent of claim_size and occurrence_period)
notidel_param <- function(claim_size, occurrence_period) {
  ...
  target_mean <- 0.4930517  #Fixed size to simulate
  target_cv <- 1.919494     #Fixed coeff. of variation
...
setldel_param <- function(claim_size, occurrence_period) {
  ...
  # Specify the target Weibull mean and target coefficient of variation
  target_mean <- 6.575582   #Fixed size to simulate
  target_cv <- 1.02259      #Fixed coeff. of variation
...
# Set to have no superimposed inflation in payment time
